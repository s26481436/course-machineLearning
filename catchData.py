...
mongoDBConnection   = MongoClient('ipaddress', 'port', username='name',password='pwd')
...
for cache in collection.find({},{"_id": 1,field1:1}):
    ...
    i=i+1
    '''
    #set limit as 10000 for test code has no error
    if i >= 10000:
        break
    '''
...
test_x = text[len(train_x) +1 : -1]
test_y = labels[len(train_x) +1 : -1]
